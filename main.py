...
class Snake:
    ...
    def draw(self):
        for index, segment in enumerate(self.body):
            ...
            if index == 0:
                pygame.draw.rect(
                    screen, RED, segment_rect, 0, 7
                )  # Draw the head in red color
            else:
...
class Game:
    ...
    def check_collision_with_edges(self):
        if (
            self.snake.body[0].x >= number_of_cells
            or self.snake.body[0].x < 0
            or self.snake.body[0].y >= number_of_cells
...
pygame.display.set_icon(pygame.image.load("Graphics/snake.png"))
...
async def main():
    ...
    while True:
        ...
        if game.state == "STOPPED":
            ...
            restart_surface = score_font.render("Press any key to restart", True, WHITE)
            screen.blit(
                game_over_surface,
                (
                    screen.get_width() // 2 - game_over_surface.get_width() // 2,
            ...
            screen.blit(
                restart_surface,
                (
                    screen.get_width() // 2 - restart_surface.get_width() // 2,
